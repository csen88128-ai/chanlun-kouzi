...
def make_decision_v2(
    ...
    try:
        ...
        # 归一化得分到 0-100
        normalized_score = max(0, min(100, (score + 50) / 1))
    ...
    if decision in ["买入", "偏多"]:
        ...
        if zhongshu:
            ...
            if gg > current_price:
                ...
                # 中枢都在现价下方，向上推算
                tp1 = round(current_price * 1.02, 2)  # +2%
            ...
            if latest_top and latest_top > current_price:
                ...
                tp2 = round(current_price * 1.04, 2)  # +4%
            ...
            if dd < current_price:
                ...
                sl1 = round(current_price * 0.97, 2)  # -3%
                plan["stop_loss"].append({"level": 1, "price": sl1, "target": "回调3%"})

            sl2 = round(current_price * 0.95, 2)  # -5%
        ...
        if zhongshu:
            ...
            if dd < current_price:
                ...
                tp1 = round(current_price * 0.98, 2)  # -2%
            ...
            if latest_bottom and latest_bottom < current_price:
                ...
                tp2 = round(current_price * 0.96, 2)  # -4%
            ...
            if gg > current_price:
                ...
                sl1 = round(current_price * 1.03, 2)  # +3%
...
# 工作流程
1. 收集各智能体的分析结果
2. 调用 make_decision_v2 进行综合评分和交易计划生成
3. 生成最终决策
4. 给出操作建议和风险提示
